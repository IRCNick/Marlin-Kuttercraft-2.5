...
uint8_t lcdDrawUpdate = LCDVIEW_CLEAR_CALL_REDRAW; // Set when the LCD needs to draw, decrements after every draw. Set to 2 in LCD routines so the LCD gets at least 1 full redraw (first redraw is partial)
  ...
  static void lcd_control_menu();
    ...
    static void lcd_control_temperature_menu_kutter();
...
void kill_screen(const char* lcd_msg) {
  ...
  static void lcd_main_menu() {
    ...
    MENU_ITEM(submenu, MSG_TMPERATURE, lcd_control_temperature_menu_kutter);

    #if ENABLED(LUZ_LED_KUTTERCARFT)
      if(ledKuttercraft){
        MENU_ITEM(function, MSG_LED_OFF, led_prende_apaga);
      }else{
        MENU_ITEM(function, MSG_LED_ON, led_prende_apaga);
      }
    #endif
  ...
  */
  ...
  void parte_seis() {
    ...
    u8g.drawStr(27,30,"Posicion 4/4");
    u8g.drawStr(20,40,"Calibre la base");
    u8g.setFont(u8g_font_5x8);
    u8g.drawStr(5,62,"Precione para continuar");
    if(LCD_CLICKED){
      while (LCD_CLICKED);
      //lcd_goto_screen(parte_cuatro);
      //lcd_main_menu();
      enqueue_and_echo_commands_P(PSTR("G1 Z10\nG28 X\nG28 Y"));
  ...
  void parte_cinco() {
    ...
    enqueue_and_echo_commands_P(PSTR("G1 Z10\nG1 X170 Y170 F4800\nG28 Z"));
    u8g.setFont(u8g_font_6x10);
    u8g.drawStr(27,30,"Posicion 3/4");
    u8g.drawStr(20,40,"Calibre la base");
    u8g.setFont(u8g_font_5x8);
    u8g.drawStr(5,62,"Precione para continuar");
    //while(LCD_CLICKED);
  ...
  void parte_cuatro() {
    ...
    enqueue_and_echo_commands_P(PSTR("G1 Z10\nG1 X170 Y40 F4800\nG28 Z"));
    u8g.setFont(u8g_font_6x10);
    u8g.drawStr(27,30,"Posicion 2/4");
    u8g.drawStr(20,40,"Calibre la base");
    u8g.setFont(u8g_font_5x8);
    u8g.drawStr(5,62,"Precione para continuar");
    if(LCD_CLICKED){
      while (LCD_CLICKED);
      lcd_goto_screen(parte_cinco);
    }
  }
  void parte_tres() {
    if (lcdDrawUpdate);
    enqueue_and_echo_commands_P(PSTR("G1 Z10\nG1 X40 Y40 F4800\nG28 Z"));
    u8g.setFont(u8g_font_6x10);
    u8g.drawStr(27,30,"Posicion 1/4");
    u8g.drawStr(20,40,"Calibre la base");
    u8g.setFont(u8g_font_5x8);
    u8g.drawStr(5,62,"Precione para continuar");
    if (LCD_CLICKED) {
  ...
  void parte_dos() {
    ...
    u8g.drawStr(10,30,"Llevando al origen");
      ...
      if (axis_homed[X_AXIS] && axis_homed[Y_AXIS] && axis_homed[Z_AXIS])
        lcd_goto_screen(parte_tres);
  }

  void calibrar() {
    defer_return_to_status = true;
    enqueue_and_echo_commands_P(PSTR("G28"));
    lcd_goto_screen(parte_dos);
  }
    ...
    #endif
    ;
    lcd_implementation_drawedit(PSTR("Llevando al origen"));
    enqueue_and_echo_commands_P(PSTR("G28 X\nG28 Y"));
    if(axis_homed[X_AXIS] && axis_homed[Y_AXIS]){
      lcd_implementation_drawedit(PSTR("Primer punto 1/5"));
      if(LCD_CLICKED){
        lcd_implementation_drawedit(PSTR("Primer punto 1/5"));
      }
    }
  ...
  static void lcd_prepare_menu() {
    ...
    */
    ...
    //
    //Menu Kuttercraft
    MENU_ITEM(submenu, MSG_AUTO_HOME_HOME, mover_eje);
    //MENU_ITEM(submenu, MSG_CALIBRAR, calibrar);
    ...
    */
    ...
    MENU_ITEM(submenu, MSG_MOVE_AXIS, lcd_move_menu_kuttercraft);
    //Kuttercraft
    #if ENABLED(AUTO_BED_LEVELING_FEATURE)
      MENU_ITEM(submenu, MSG_OFFSET, calibrar_Offset);
      //MENU_ITEM(submenu, MSG_LEVEL_BED, lcd_level_bed);
    #endif
  ...
  //Offset Kuttercraft (Ayuda Grafica)
  /*
  static void quinientos(){
    if (lcdDrawUpdate);
    enqueue_and_echo_commands_P(PSTR("M500"));
    defer_return_to_status = true;
    lcd_control_menu();
  }
  static void quinientosuno(){
    if (lcdDrawUpdate);
  ...
  */
  /*
  static void offset_Guardar(){
    START_MENU();
    MENU_ITEM(submenu, MSG_ZPROBE_GUARDAR, quinientos);
    MENU_ITEM(submenu, MSG_ZPROBE_CANCELAR, quinientosuno);
    END_MENU();
  }
  */
  #if ENABLED(AUTO_BED_LEVELING_FEATURE)
  ...
  */
   ...
   */
    ...
    static void lcd_control_temperature_menu_kutter() {
      ...
      MENU_ITEM(function, MSG_PREHEAT_PLA, pla_k);
      MENU_ITEM(function, MSG_PREHEAT_ABS, abs_k);
      #if TEMP_SENSOR_0 != 0 && DISABLED(LASERBEAM)
        #if TEMP_SENSOR_1 != 0 || TEMP_SENSOR_2 != 0 || TEMP_SENSOR_3 != 0 || TEMP_SENSOR_BED != 0
          //MENU_ITEM(submenu, MSG_PREHEAT_PLA, lcd_preheat_pla_menu);
          //MENU_ITEM(submenu, MSG_PREHEAT_ABS, lcd_preheat_abs_menu);
